...
class ResNetBlock1D(nn.Module):
    def __init__(self,in_channels,out_channels,stride=1,dropout=0.3):
        ...
        self.downsample=nn.Sequential(nn.Conv1d(in_channels,out_channels,kernel_size=1,stride=stride,bias=False),nn.BatchNorm1d(out_channels)) if stride!=1 or in_channels!=out_channels else nn.Identity()
    def forward(self,x):
...
class ResNet1D(nn.Module):
    def __init__(self,in_channels=24,num_classes=8,channels=(64,128,256),blocks_per_stage=(2,2,2),dropout=0.3):
        super().__init__()
        self.stem=nn.Sequential(nn.Conv1d(in_channels,channels[0],kernel_size=7,stride=1,padding=3,bias=False),nn.BatchNorm1d(channels[0]),nn.ReLU(inplace=True),nn.MaxPool1d(kernel_size=2,stride=2))
        stages=[]
        in_c=channels[0]
        for stage_idx,(out_c,n_blocks) in enumerate(zip(channels,blocks_per_stage)):
            stride=1 if stage_idx==0 else 2
            blocks=[ResNetBlock1D(in_c,out_c,stride=stride,dropout=dropout)]
            for _ in range(1,n_blocks):
                blocks.append(ResNetBlock1D(out_c,out_c,stride=1,dropout=dropout))
...
def main(args):
    ...
    model=ResNet1D(in_channels=24,num_classes=num_classes).to(device)
...
if __name__=="__main__":
    parser=argparse.ArgumentParser(description="Train ResNet1D")
    parser.add_argument("--npz_path",type=str,default="gait1_preprocessed.npz")
    parser.add_argument("--checkpoint_path",type=str,default="best_resnet1d.pt")
