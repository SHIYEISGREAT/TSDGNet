...
class CNNLSTM(nn.Module):
    def __init__(self,in_channels=24,num_classes=8,conv_channels=(64,128,128),lstm_hidden=128,lstm_layers=2,bidirectional=True,dropout=0.3):
        super().__init__()
        c1,c2,c3=conv_channels
        self.stem=nn.Sequential(nn.Conv1d(in_channels,c1,kernel_size=7,stride=2,padding=3,bias=False),nn.BatchNorm1d(c1),nn.ReLU(inplace=True),nn.MaxPool1d(kernel_size=2),nn.Conv1d(c1,c2,kernel_size=5,stride=2,padding=2,bias=False),nn.BatchNorm1d(c2),nn.ReLU(inplace=True),nn.Conv1d(c2,c3,kernel_size=3,stride=2,padding=1,bias=False),nn.BatchNorm1d(c3),nn.ReLU(inplace=True))
        self.lstm=nn.LSTM(input_size=c3,hidden_size=lstm_hidden,num_layers=lstm_layers,batch_first=True,dropout=dropout if lstm_layers>1 else 0.0,bidirectional=bidirectional)
        feat_dim=lstm_hidden*(2 if bidirectional else 1)
        self.classifier=nn.Sequential(nn.Linear(feat_dim,256),nn.ReLU(inplace=True),nn.Dropout(0.5),nn.Linear(256,num_classes))
    def forward(self,x):
...
def main(args):
    ...
    model=CNNLSTM(in_channels=24,num_classes=num_classes).to(device)
...
if __name__=="__main__":
    parser=argparse.ArgumentParser(description="Train CNNLSTM")
    parser.add_argument("--npz_path",type=str,default="gait1_preprocessed.npz")
    parser.add_argument("--checkpoint_path",type=str,default="best_cnn_lstm.pt")
