...
class TCNBlock(nn.Module):
    def __init__(self,in_channels,out_channels,kernel_size=5,dilation=1,dropout=0.3):
        ...
        self.conv1=nn.Conv1d(in_channels,out_channels,kernel_size=kernel_size,padding=padding,dilation=dilation,bias=False)
        self.bn1=nn.BatchNorm1d(out_channels)
        self.conv2=nn.Conv1d(out_channels,out_channels,kernel_size=kernel_size,padding=padding,dilation=dilation,bias=False)
        self.bn2=nn.BatchNorm1d(out_channels)
        self.relu=nn.ReLU(inplace=True)
        self.dropout=nn.Dropout(dropout)
        self.downsample=nn.Sequential(nn.Conv1d(in_channels,out_channels,kernel_size=1,bias=False),nn.BatchNorm1d(out_channels)) if in_channels!=out_channels else nn.Identity()
    def forward(self,x):
...
class TCN(nn.Module):
    def __init__(self,in_channels=24,num_classes=8,channels=(64,128,256),blocks_per_stage=(2,2,2),kernel_size=5,dropout=0.3):
        super().__init__()
        self.stem=nn.Sequential(nn.Conv1d(in_channels,channels[0],kernel_size=kernel_size,padding=kernel_size//2,bias=False),nn.BatchNorm1d(channels[0]),nn.ReLU(inplace=True))
        stages=[]
        in_c=channels[0]
        dilation=1
        for out_c,n_blocks in zip(channels,blocks_per_stage):
            blocks=[]
            if in_c!=out_c:
                blocks.append(TCNBlock(in_c,out_c,kernel_size=kernel_size,dilation=dilation,dropout=dropout))
                in_c=out_c
                dilation*=2
        ...
        self.classifier=nn.Sequential(nn.Linear(channels[-1],256),nn.ReLU(inplace=True),nn.Dropout(0.5),nn.Linear(256,num_classes))
    def forward(self,x):
        feat=self.stages(self.stem(x))
        return self.classifier(self.gap(feat).squeeze(-1))
def main(args):
    set_seed(args.seed)
    ...
    model=TCN(in_channels=24,num_classes=num_classes).to(device)
...
if __name__=="__main__":
    parser=argparse.ArgumentParser(description="Train TCN")
    parser.add_argument("--npz_path",type=str,default="gait1_preprocessed.npz")
    parser.add_argument("--checkpoint_path",type=str,default="best_tcn.pt")
