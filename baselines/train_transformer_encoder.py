...
class SinusoidalPositionalEncoding(nn.Module):
    ...
    def forward(self,x):
        t=x.size(1)
        if t>self.pe.size(1):
            raise ValueError("Sequence length exceeds maximum positional length")
        return x+self.pe[:,:t,:]
class TransformerEncoder1D(nn.Module):
    def __init__(self,in_channels=24,num_classes=8,embed_dim=128,num_heads=4,num_layers=3,ff_dim=256,dropout=0.1):
        super().__init__()
        self.stem=nn.Sequential(nn.Conv1d(in_channels,64,kernel_size=7,stride=2,padding=3,bias=False),nn.BatchNorm1d(64),nn.ReLU(inplace=True),nn.MaxPool1d(kernel_size=2),nn.Conv1d(64,embed_dim,kernel_size=5,stride=2,padding=2,bias=False),nn.BatchNorm1d(embed_dim),nn.ReLU(inplace=True),nn.Conv1d(embed_dim,embed_dim,kernel_size=3,stride=2,padding=1,bias=False),nn.BatchNorm1d(embed_dim),nn.ReLU(inplace=True))
        self.pos_encoder=SinusoidalPositionalEncoding(embed_dim,max_len=4096)
        layer=nn.TransformerEncoderLayer(d_model=embed_dim,nhead=num_heads,dim_feedforward=ff_dim,dropout=dropout,activation="gelu",batch_first=True,norm_first=True)
        self.encoder=nn.TransformerEncoder(layer,num_layers=num_layers,norm=nn.LayerNorm(embed_dim))
        self.classifier=nn.Sequential(nn.Linear(embed_dim,256),nn.ReLU(inplace=True),nn.Dropout(0.5),nn.Linear(256,num_classes))
    def forward(self,x):
        feat=self.stem(x).transpose(1,2)
        feat=self.encoder(self.pos_encoder(feat))
...
def main(args):
    ...
    model=TransformerEncoder1D(in_channels=24,num_classes=num_classes).to(device)
...
if __name__=="__main__":
    parser=argparse.ArgumentParser(description="Train TransformerEncoder1D")
    parser.add_argument("--npz_path",type=str,default="gait1_preprocessed.npz")
    parser.add_argument("--checkpoint_path",type=str,default="best_transformer_encoder.pt")
