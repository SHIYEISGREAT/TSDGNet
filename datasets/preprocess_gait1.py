...
def resolve_dataset_root(path: str) -> Path:
    root = to_abs_path(path)
    candidates = [root, root / "dataset", root / "dataset" / "dataset"]
    for candidate in candidates:
        if (candidate / "data").is_dir():
            return candidate
    raise RuntimeError(f"data folder not found under: {root}")
...
def load_imus_for_trial(trial_dir: str, trial_id: str) -> Tuple[np.ndarray, float, Dict[str, Any]]:
    ...
    usable = (ncol // 6) * 6
    if usable < 24:
        raise ValueError(f"{proc_path} has {ncol} numeric columns, cannot form 4x6 IMU channels.")
...
def main(dataset_root: str, output_dir: str, trim_sec: float = 1.0, target_fs: float = 50.0) -> None:
    dataset_root_path = resolve_dataset_root(dataset_root)
    output_dir_path = to_abs_path(output_dir)
    data_root = dataset_root_path / "data"
    trials = discover_trials(str(data_root))
    ...
    max_len = 0
    skipped = 0
    for cohort, trial_dir, trial_id in tqdm(trials, desc="processing"):
        try:
            imu_raw, fs, meta = load_imus_for_trial(trial_dir, trial_id)
        except FileNotFoundError as e:
            print(f"skip={trial_id}; reason={e}")
            skipped += 1
            continue
    ...
    N = len(all_signals)
    print(f"N={N}; max_len={max_len}; skipped={skipped}")
    if N == 0:
    ...
    out_path = output_dir_path / "gait1_preprocessed.npz"
    np.savez_compressed(
        out_path,
        X=X,
        lengths=lengths,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--trim_sec", type=float, default=1.0)
