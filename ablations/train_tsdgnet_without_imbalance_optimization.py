...
class TSDGNet(nn.Module):
    def __init__(self,in_channels=24,num_classes=8,backbone_channels=(64,128,256),d_model=64,num_graph_layers=1,num_heads=4,alpha_init=0.3):
        ...
        self.classifier=nn.Sequential(nn.Linear(c3+d_model,256),nn.ReLU(inplace=True),nn.Dropout(0.5),nn.Linear(256,num_classes))
    def forward(self,x):
        ...
        return self.classifier(torch.cat([g_backbone,g_graph],dim=-1))
...
if __name__=="__main__":
    ...
    parser.add_argument("--checkpoint_path",type=str,default="best_tsdgnet_no_imbalance.pt")
