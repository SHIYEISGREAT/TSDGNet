...
class TSDGNet(nn.Module):
    def __init__(self,in_channels=24,num_classes=8,backbone_channels=(64,128,256)):
        super().__init__()
        ...
        self.classifier=nn.Sequential(nn.Linear(c3,256),nn.ReLU(inplace=True),nn.Dropout(0.5),nn.Linear(256,num_classes))
    def forward(self,x):
        feat=self.backbone(x)
        g=self.gap(feat).squeeze(-1)
...
def main(args):
    ...
    model=TSDGNet(in_channels=24,num_classes=num_classes,backbone_channels=(64,128,256)).to(device)
...
if __name__=="__main__":
    ...
    parser.add_argument("--checkpoint_path",type=str,default="best_tsdgnet_temporal_only.pt")
