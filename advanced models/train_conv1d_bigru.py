...
class Conv1DBiGRUBaseline(nn.Module):
    def __init__(
        self,
        in_channels: int = 24,
        num_classes: int = 8,
        ...
        if temporal_pool not in ("last", "mean"):
            raise ValueError("temporal_pool must be 'last' or 'mean'")
        c1, c2, c3 = conv_channels
        padding = kernel_size // 2
        self.temporal_pool = temporal_pool
        self.conv1 = nn.Conv1d(in_channels, c1, kernel_size=kernel_size, padding=padding, bias=False)
        self.bn1 = nn.BatchNorm1d(c1)
        self.pool1 = nn.MaxPool1d(kernel_size=pool_kernel_size)
        self.conv2 = nn.Conv1d(c1, c2, kernel_size=kernel_size, padding=padding, bias=False)
        self.bn2 = nn.BatchNorm1d(c2)
        self.pool2 = nn.MaxPool1d(kernel_size=pool_kernel_size)
        self.conv3 = nn.Conv1d(c2, c3, kernel_size=kernel_size, padding=padding, bias=False)
        ...
        self.feature_dim = gru_hidden_size * 2
    ...
    def _forward_backbone(self, x):
        ...
        return x
    def _forward_sequence_features(self, x):
        feat = self._forward_backbone(x)
        feat_seq = feat.transpose(1, 2).contiguous()
        seq_out, _ = self.bigru(feat_seq)
        if self.temporal_pool == "last":
            g = seq_out[:, -1, :]
        else:
            g = seq_out.mean(dim=1)
        return g
    def forward(self, x):
        g = self._forward_sequence_features(x)
        logits = self.classifier(g)
        return logits
    def extract_feature_vector(self, x):
        return self._forward_sequence_features(x)
def create_model(num_classes,args):
    return Conv1DBiGRUBaseline(in_channels=24,num_classes=num_classes)
def main(args):
    set_seed(args.seed)
...
if __name__=="__main__":
    parser=argparse.ArgumentParser(description="Train Conv1DBiGRUBaseline")
    parser.add_argument("--npz_path",type=str,default="gait1_preprocessed.npz")
    parser.add_argument("--checkpoint_path",type=str,default="conv1d_bigru.pth")
